...
void Stats::request_stats(const std::set<StatIndex> &wanted_stats,
    ...
    for (const auto &stat_index : requested_stats) {
        if (!is_available(stat_index)) {
            LOGW(StatsProvider::default_graph_data(stat_index).name + " : not available");
        }
    }
}
...
void Stats::begin_sampling(core::CommandBuffer &cb) {
...
void Stats::end_sampling(core::CommandBuffer &cb) {
    // Inform the providers
    for (auto &p : providers) {
        p->end_sampling(cb);
    }
...
StatGraphData::StatGraphData(std::string name,
                             ...
                             float scale_factor,
                             bool has_fixed_max,
                             float max_value) : name(std::move(name)),
