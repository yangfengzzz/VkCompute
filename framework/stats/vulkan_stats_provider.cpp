...
VulkanStatsProvider::VulkanStatsProvider(std::set<StatIndex> &requested_stats,
    ...
    core::Device &device = render_context.get_device();
    const core::PhysicalDevice &gpu = device.get_gpu();
...
bool VulkanStatsProvider::create_query_pools(uint32_t queue_family_index) {
    ...
    query_pool = std::make_unique<core::QueryPool>(device, pool_create_info);
    ...
    if (has_timestamps) {
        ...
        timestamp_pool = std::make_unique<core::QueryPool>(device, timestamp_pool_create_info);
    }
...
void VulkanStatsProvider::begin_sampling(core::CommandBuffer &cb) {
...
void VulkanStatsProvider::end_sampling(core::CommandBuffer &cb) {
    uint32_t active_frame_idx = render_context.get_active_frame_index();
