...
Device &FrameResource::get_device() {
    return device;
}
...
std::vector<std::unique_ptr<CommandPool>> &FrameResource::get_command_pools(const Queue &queue, CommandBuffer::ResetMode reset_mode) {
...
std::vector<uint32_t> FrameResource::collect_bindings_to_update(const DescriptorSetLayout &descriptor_set_layout,
...
const FencePool &FrameResource::get_fence_pool() const {
    return fence_pool;
}
...
const SemaphorePool &FrameResource::get_semaphore_pool() const {
...
VkDescriptorSet FrameResource::request_descriptor_set(const DescriptorSetLayout &descriptor_set_layout,
                                                      const BindingMap<VkDescriptorBufferInfo> &buffer_infos,
                                                      const BindingMap<VkDescriptorImageInfo> &image_infos,
...
void FrameResource::update_descriptor_sets(size_t thread_index) {
    assert(thread_index < descriptor_sets.size());
...
void FrameResource::clear_descriptors() {
