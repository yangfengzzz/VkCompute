...
public:
	 ...
	 */
    uint64_t add_triangle_geometry(std::unique_ptr<Buffer> &vertex_buffer,
    ...
    void update_triangle_geometry(uint64_t triangleUUID, std::unique_ptr<Buffer> &vertex_buffer,
                                  std::unique_ptr<Buffer> &index_buffer,
                                  std::unique_ptr<Buffer> &transform_buffer,
                                  uint32_t triangle_count,
                                  uint32_t max_vertex,
	 ...
	 * @param flags Ray tracing geometry flags
	 */
    uint64_t add_instance_geometry(std::unique_ptr<Buffer> &instance_buffer,
                                   uint32_t instance_count,
                                   uint32_t transform_offset = 0,
                                   VkGeometryFlagsKHR flags = VK_GEOMETRY_OPAQUE_BIT_KHR);

    void update_instance_geometry(uint64_t instance_UID, std::unique_ptr<Buffer> &instance_buffer,
                                  uint32_t instance_count,
                                  uint32_t transform_offset = 0,
                                  VkGeometryFlagsKHR flags = VK_GEOMETRY_OPAQUE_BIT_KHR);
