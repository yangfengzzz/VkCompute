...
void CommandBuffer::bind_buffer(const Buffer &buffer, VkDeviceSize offset, VkDeviceSize range,
...
void CommandBuffer::bind_image(const ImageView &image_view, const Sampler &sampler,
                               uint32_t set, uint32_t binding, uint32_t array_element) {
...
void CommandBuffer::bind_vertex_buffers(uint32_t first_binding,
    ...
    std::transform(buffers.begin(), buffers.end(), buffer_handles.begin(),
                   [](const Buffer &buffer) { return buffer.get_handle(); });
...
void CommandBuffer::draw_indexed_indirect(const Buffer &buffer, VkDeviceSize offset, uint32_t draw_count, uint32_t stride) {
    flush(VK_PIPELINE_BIND_POINT_GRAPHICS);
...
void CommandBuffer::update_buffer(const Buffer &buffer, VkDeviceSize offset, const std::vector<uint8_t> &data) {
    vkCmdUpdateBuffer(get_handle(), buffer.get_handle(), offset, data.size(), data.data());
}

void CommandBuffer::blit_image(const Image &src_img, const Image &dst_img, const std::vector<VkImageBlit> &regions) {
...
void CommandBuffer::resolve_image(const Image &src_img, const Image &dst_img, const std::vector<VkImageResolve> &regions) {
    vkCmdResolveImage(get_handle(), src_img.get_handle(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                      dst_img.get_handle(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                      to_u32(regions.size()), regions.data());
}

void CommandBuffer::copy_buffer(const Buffer &src_buffer, const Buffer &dst_buffer, VkDeviceSize size) {
    VkBufferCopy copy_region = {};
    copy_region.size = size;
    vkCmdCopyBuffer(get_handle(), src_buffer.get_handle(), dst_buffer.get_handle(), 1, &copy_region);
}

void CommandBuffer::copy_image(const Image &src_img, const Image &dst_img, const std::vector<VkImageCopy> &regions) {
    vkCmdCopyImage(get_handle(), src_img.get_handle(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                   dst_img.get_handle(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                   to_u32(regions.size()), regions.data());
}

void CommandBuffer::copy_buffer_to_image(const Buffer &buffer, const Image &image,
...
void CommandBuffer::copy_image_to_buffer(const Image &image, VkImageLayout image_layout, const Buffer &buffer,
                                         const std::vector<VkBufferImageCopy> &regions) {
