...
void MonteCarloPiSimulation::step_simulation(float time, CudaStream &stream) {
    checkCudaErrors(cudaMemsetAsync(m_numPointsInCircle, 0,
                                    sizeof(*m_numPointsInCircle), stream.get_handle()));

    monte_carlo_kernel<<<m_blocks, m_threads, 0, stream.get_handle()>>>(
    ...
    checkCudaErrors(cudaMemcpyAsync(m_hostNumPointsInCircle, m_numPointsInCircle,
                                    ...
                                    cudaMemcpyDeviceToHost, stream.get_handle()));

    // Queue up a stream callback to compute and print the PI value.
    checkCudaErrors(
        cudaLaunchHostFunc(stream.get_handle(), this->compute_pi_callback, (void *)this));
}

void MonteCarloPiSimulation::compute_pi_callback(void *args) {
    auto *cbData = (MonteCarloPiSimulation *)args;
...
//----------------------------------------------------------------------------------------------------------------------------------------------
MonteCarloPiSimulation::MonteCarloPiSimulation(size_t num_points, CudaDevice &device)
    : m_xyVector(nullptr),
    ...
    checkCudaErrors(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &m_blocks, monte_carlo_kernel, device.get_prop().warpSize, 0));
    ...
    setup_simulation_allocations(device);
...
MonteCarloPiSimulation::~MonteCarloPiSimulation() {
    ...
    cleanup_simulation_allocations();
}

void MonteCarloPiSimulation::setup_simulation_allocations(CudaDevice &device) {
    ...
    allocProp.location.id = device.get_cuda_device();
    ...
    get_default_security_descriptor(&allocProp);
    ...
    accessDescriptor.location.id = device.get_cuda_device();
...
void MonteCarloPiSimulation::cleanup_simulation_allocations() {
