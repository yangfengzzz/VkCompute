...
static inline void vec4_scale(vec4 r, const vec4 v, float s) {
    int i;
...
static inline void mat4x4_perspective(mat4x4 m, float y_fov, float aspect, float n, float f) {
    ...
    auto const a = (float)(1.f / tan(y_fov / 2.f));
...
static inline void quat_add(quat r, const quat a, const quat b) {
    int i;
    for (i = 0; i < 4; ++i) r[i] = a[i] + b[i];
}
static inline void quat_sub(quat r, quat a, const quat b) {
    int i;
    for (i = 0; i < 4; ++i) r[i] = a[i] - b[i];
}
...
static inline void quat_scale(quat r, const quat v, float s) {
    int i;
    for (i = 0; i < 4; ++i) r[i] = v[i] * s;
}
static inline float quat_inner_product(const quat a, const quat b) {
    float p = 0.f;
    int i;
    for (i = 0; i < 4; ++i) p += b[i] * a[i];
    return p;
