...
bool bsr_fixed_block_is_zero(const T *val, int value_size) {
    return std::all_of(val, val + N, [](float v) { return v == T(0); });
}

template<typename T>
bool bsr_dyn_block_is_zero(const T *val, int value_size) {
    return std::all_of(val, val + value_size, [](float v) { return v == T(0); });
}

template<int N, typename T>
void bsr_fixed_block_accumulate(const T *val, T *sum, int value_size) {
...
template<typename T>
void bsr_dyn_block_accumulate(const T *val, T *sum, int value_size) {
...
template<typename T>
void bsr_dyn_block_transpose(const T *src, T *dest, int row_count, int col_count) {
