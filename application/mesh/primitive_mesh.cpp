...
namespace vox {
ModelMeshPtr PrimitiveMesh::create_sphere(float radius, size_t segments, bool no_longer_accessible) {
...
ModelMeshPtr PrimitiveMesh::create_cuboid(float width, float height, float depth, bool no_longer_accessible) {
    auto mesh = MeshManager::get_singleton().load_model_mesh();
...
ModelMeshPtr PrimitiveMesh::create_plane(
...
ModelMeshPtr PrimitiveMesh::create_cylinder(float radius_top, float radius_bottom, float height, size_t radial_segments, size_t height_segments, bool no_longer_accessible) {
    auto mesh = MeshManager::get_singleton().load_model_mesh();
...
ModelMeshPtr PrimitiveMesh::create_torus(float radius, float tube_radius, size_t radial_segments, size_t tubular_segments, float arc, bool no_longer_accessible) {
    auto mesh = MeshManager::get_singleton().load_model_mesh();
...
ModelMeshPtr PrimitiveMesh::create_cone(float radius, float height, size_t radial_segments, size_t height_segments, bool no_longer_accessible) {
    auto mesh = MeshManager::get_singleton().load_model_mesh();
...
ModelMeshPtr PrimitiveMesh::create_capsule(float radius, float height, size_t radial_segments, size_t height_segments, bool no_longer_accessible) {
    auto mesh = MeshManager::get_singleton().load_model_mesh();
    ...
    PrimitiveMesh::create_capsule_cap(radius, height, radial_segments, kDoublePi, kTorsoVertexCount, 1, positions,
                                      normals, uvs, indices, indices_offset);

    PrimitiveMesh::create_capsule_cap(radius, height, radial_segments, -kDoublePi, kTorsoVertexCount + kCapVertexCount,
...
void PrimitiveMesh::create_capsule_cap(float radius, float height, size_t radial_segments, float cap_alpha_range, size_t offset, ssize_t pos_index, std::vector<Vector3F> &positions, std::vector<Vector3F> &normals, std::vector<Vector2F> &uvs, std::vector<uint32_t> &indices, size_t indices_offset) {
    const auto kRadialCount = radial_segments + 1;
    const auto kHalfHeight = height * 0.5f;
    const auto kCapVertexCount = kRadialCount * kRadialCount;
    const auto kCapRectangleCount = radial_segments * radial_segments;
...
void PrimitiveMesh::initialize(const ModelMeshPtr &mesh,
    ...
    mesh->upload_data(no_longer_accessible);
    mesh->add_sub_mesh(0, static_cast<uint32_t>(indices.size()));
}
