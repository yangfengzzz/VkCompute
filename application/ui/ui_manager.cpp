...
namespace vox {
ui::UiManager *ui::UiManager::get_singleton_ptr() { return ms_singleton; }
...
UiManager::UiManager(GLFWwindow *glfw_window, rendering::RenderContext *render_context, Style style)
    ...
    ImGui_ImplVulkan_LoadFunctions(
        [](const char *function_name, void *user_data) {
            ...
            return vkGetInstanceProcAddr(app->render_context_->get_device().get_gpu().get_instance().get_handle(),
                                         function_name);
        },
        this);
...
void UiManager::set_editor_layout_auto_save_frequency(float frequency) { ImGui::GetIO().IniSavingRate = frequency; }

float UiManager::editor_layout_autosave_frequency() { return ImGui::GetIO().IniSavingRate; }
...
void UiManager::setup_render_pass() {
    ...
    VK_CHECK(vkCreateRenderPass(render_context_->get_device().get_handle(), &render_pass_create_info, nullptr,
                                &render_pass_));
}
...
void UiManager::draw(core::CommandBuffer &command_buffer) {
    ImGui::Render();
