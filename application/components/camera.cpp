...
Camera::Camera(Entity *entity)
    : Component(entity), shader_data_(entity->get_scene()->get_device()), camera_property_("cameraData") {
...
const BoundingFrustum &Camera::get_frustum() const { return frustum_; }

float Camera::get_near_clip_plane() const { return near_clip_plane_; }
...
float Camera::get_far_clip_plane() const { return far_clip_plane_; }
...
float Camera::get_field_of_view() const { return field_of_view_; }
...
Vector4F Camera::get_viewport() const { return viewport_; }
...
bool Camera::is_orthographic() const { return is_orthographic_; }
...
float Camera::get_orthographic_size() const { return orthographic_size_; }
...
Matrix4x4F Camera::get_view_matrix() {
    // Remove scale
    if (is_view_matrix_dirty_->flag_) {
        is_view_matrix_dirty_->flag_ = false;
        view_matrix_ = transform_->get_world_matrix().inverse();
    }
    return view_matrix_;
}
...
Matrix4x4F Camera::get_projection_matrix() {
    ...
    if (!is_orthographic_) {
        projection_matrix_ =
            makePerspective(degreesToRadians(field_of_view_), get_aspect_ratio(), near_clip_plane_, far_clip_plane_);
    } else {
        const auto kWidth = orthographic_size_ * get_aspect_ratio();
...
bool Camera::is_hdr_enabled() { return false; }

void Camera::enable_hdr(bool value) { assert(false && "not implementation"); }
...
Vector4F Camera::world_to_viewport_point(const Point3F &point) {
    auto temp_mat_4 = get_projection_matrix() * get_view_matrix();
...
Vector2F Camera::screen_to_viewport_point(const Vector2F &point) const {
    const Vector4F kViewport = this->get_viewport();
...
void Camera::on_active() { get_entity()->get_scene()->attach_render_camera(this); }

void Camera::on_inactive() { get_entity()->get_scene()->detach_render_camera(this); }
...
void Camera::update() {
    camera_data_.view_mat = get_view_matrix();
    camera_data_.proj_mat = get_projection_matrix();
    camera_data_.vp_mat = get_projection_matrix() * get_view_matrix();
    camera_data_.view_inv_mat = transform_->get_world_matrix();
    camera_data_.proj_inv_mat = inverse_projection_matrix();
    camera_data_.camera_pos = transform_->get_world_position();
...
Matrix4x4F Camera::inv_view_proj_mat() {
    if (is_inv_view_proj_dirty_->flag_) {
        ...
        inv_view_proj_mat_ = transform_->get_world_matrix() * inverse_projection_matrix();
    }
...
Matrix4x4F Camera::inverse_projection_matrix() {
    if (is_inv_proj_mat_dirty_) {
        ...
        inverse_projection_matrix_ = get_projection_matrix().inverse();
    }
...
uint32_t Camera::get_width() const { return width_; }

uint32_t Camera::get_height() const { return height_; }

uint32_t Camera::get_framebuffer_width() const { return fb_width_; }

uint32_t Camera::get_framebuffer_height() const { return fb_height_; }
