...
BaseMaterial::BaseMaterial(core::Device &device, const std::string &name)
    ...
    if (last_sampler_create_info_.sType != VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO) {
        ...
        last_sampler_create_info_.maxAnisotropy =
            device.get_gpu().get_features().samplerAnisotropy ? (device.get_gpu().get_properties().limits.maxSamplerAnisotropy) : 1.0f;
